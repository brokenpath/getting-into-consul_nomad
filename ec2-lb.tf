...
## Application Load Balancer - Nomad Server
resource "aws_lb" "alb_nomad" {
  name_prefix        = "nomul-" # 6 character length
  load_balancer_type = "application"
  security_groups    = [aws_security_group.load_balancer.id]
  subnets            = aws_subnet.public.*.id
  idle_timeout       = 60
...
## Target Group
resource "aws_lb_target_group" "alb_nomad_targets" {
  name_prefix          = "nomul-"
  port                 = 4646
  protocol             = "HTTP"
  vpc_id               = aws_vpc.consul.id
  ...
  health_check {
    enabled             = true
    interval            = 10
    path                = "/v1/status/leader" // the server API health port?
    protocol            = "HTTP"              // switch to HTTPS?
    timeout             = 5
    healthy_threshold   = 3
    unhealthy_threshold = 3
...
resource "aws_lb_listener" "alb_nomad" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.alb_nomad_targets.arn
  }
}